...
##Grafico de linha notificaçoes por mês

```{r}
grafico_notificacoes_mes <- dados_dengue_sudeste %>%
  mutate(
    data_notificacao = as.Date(DT_NOTIFIC),
    mes_notificacao_num = format(data_notificacao, "%m"),
    mes_notificacao = factor(
      mes_notificacao_num,
  ...
  count(mes_notificacao, name = "quantidade_notificacoes")

grf <- plot_series(
  grafico_notificacoes_mes,
  label_x = "Mês da notificação",
...
##Taxa mensal de óbito pelo agravo

```{r}
## Gráfico de linha: taxa de óbito pelo agravo por mês

grafico_taxa_obito_mes <- dados_dengue_sudeste %>%
  mutate(
    data_notificacao = as.Date(DT_NOTIFIC),
    mes_notificacao_num = format(data_notificacao, "%m"),
    mes_notificacao = factor(
...
grf <- plot_series(
  grafico_taxa_obito_mes %>%
    select(mes_notificacao, taxa_obito_percentual),
  label_x = "Mês da notificação",
...
##Grafico de quantidade de registros por faixa etaria

```{r}

grafico_faixa_etaria <- dados_dengue_sudeste %>%
  ...
  count(faixa_etaria, name = "quantidade_registros")

grf <- plot_bar(
  grafico_faixa_etaria,
  label_x = "Faixa etária",
  label_y = "Quantidade de registros",
  colors = "steelblue"
)
